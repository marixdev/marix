...
| 🔄 | **GitHub 同期** | Argon2id + AES-256-GCM | ゼロナレッジクラウドバックアップ—GitHub は暗号化されたブロブのみ保存 |
...
| システム RAM | Argon2id メモリ | セキュリティレベル |
|--------------|-----------------|-------------------|
| ≥ 8 GB | 64 MB | 高 |
| ≥ 4 GB | 32 MB | 中 |
| < 4 GB | 16 MB | 低メモリ最適化 |

アプリはシステム RAM を自動検出し、セキュリティを維持しながら最適なパフォーマンスのために暗号化パラメータを調整します。
...
```
パスワード → Argon2id(16-64MB メモリ) → 256 ビットキー → AES-256-GCM → 暗号化されたバックアップ
```
...
🛡️ **ブルートフォース耐性** — Argon2id は試行ごとに 16-64MB の RAM を必要とする  
...
| **Argon2id KDF** | 16-64MB メモリ、3 イテレーション、4 並列レーン |
...
| アルゴリズム | パラメータ |
|--------------|------------|
| **キー導出** | Argon2id（メモリ：16-64MB、イテレーション：3、並列性：4） |
| **対称暗号化** | AES-256-GCM |
| **ソルト** | 32 バイト（暗号学的ランダム） |
