...
- **LANファイル転送**: LAN経由でファイルを送信 — 送信者はファイルを選択して6桁のコードを表示、受信者はコードを入力して送信者を自動検出しファイルを受信
- **LANサーバー共有**: サーバー設定を共有 — 送信者はサーバーを選択して選んだデバイスに送信、受信者はコードを入力して復号化しインポート
...
3. **JSON ファイルをダウンロード**して `google-credentials.json` として保存
4. **Marix で接続** → アプリが認証のためにブラウザを開きます

#### 動作方法

```
[パスワード] → Argon2id KDF → AES-256-GCM → [暗号化されたファイル] → Google Drive
```

- ✅ **ゼロナレッジ**: パスワードはデバイスから離れません
- ✅ **エンドツーエンド暗号化**: Google は暗号化されたデータのみ表示
- ✅ **サーバーなし**: データは PC から Drive に直接送信
- ✅ **復元**: パスワードでどこからでも復元可能
