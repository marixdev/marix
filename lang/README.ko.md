...
Marix는 다음 보안 가정을 기반으로 설계되었습니다:

> ⚠️ **Marix는 로컬의 손상되지 않은 호스트 환경을 가정합니다.**  
> 악의적인 OS 수준 공격자나 손상된 런타임에 대한 방어를 시도하지 않습니다.

**보호 범위 내:**
- 비밀번호 없이 백업 파일 도난
- 암호화된 백업에 대한 무차별 비밀번호 공격
- 전송 또는 저장 중 데이터 변조 (AEAD를 통해 감지)
- 클라우드 제공업체의 데이터 접근 (클라이언트 측 암호화)

**보호 범위 외:**
- 기기에서 root/admin 권한을 가진 맬웨어
- 앱이 실행 중인 잠금 해제된 기기에 대한 물리적 접근
- 키로거 또는 화면 캡처 맬웨어
- 손상된 운영 체제 또는 Electron 런타임

### Marix가 하지 않는 것

| ❌ | 설명 |
|----|------|
| **원격 키 저장 없음** | 개인 키는 절대 기기를 떠나지 않습니다 |
| **키 에스크로 없음** | 어떤 상황에서도 키를 복구할 수 없습니다 |
| **비밀번호 없이 복구 불가** | 비밀번호 분실 = 백업 분실 (설계상) |
| **암호화 중 네트워크 호출 없음** | 암호화 작업은 100% 오프라인 |
| **클라우드 서버 없음** | 우리는 어떤 인프라도 운영하지 않습니다 |
| **텔레메트리 없음** | 분석 제로, 추적 제로, 데이터 수집 제로 |
...
| 🔄 | **클라우드 동기화** | Argon2id + AES-256-GCM | 클라이언트 측 암호화—클라우드 제공업체는 암호화된 블롭만 저장 |
...
Marix는 Argon2id 매개변수에 **자동 보정**을 사용합니다—이것은 응용 암호학에서 널리 채택된 모범 사례입니다:

| 기능 | 설명 |
|------|------|
| **목표 시간** | 사용자 기기에서 ~1초 (800-1200ms) |
| **자동 보정** | 첫 실행 시 메모리와 반복 횟수 자동 조정 |
| **적응형** | 약한 기기와 강력한 기기 모두에서 최적 작동 |
| **백그라운드 보정** | 앱 시작 시 실행되어 원활한 UX 제공 |
| **매개변수 저장** | KDF 매개변수가 암호화된 데이터와 함께 저장되어 다른 기기에서 복호화 가능 |
| **보안 최저선** | 최소 64MB 메모리, 2회 반복 (OWASP 47MB 초과) |

> **왜 ~1초인가?** 이것은 실용 암호학의 표준 권장사항입니다. 사용자 경험을 해치지 않으면서 강력한 무차별 대입 저항력을 제공합니다. 매개변수는 각 기기에 자동 적응—"표준" 설정을 추측할 필요가 없습니다.

### 메모리 기준선 (자동 튜닝 시작점)

| 시스템 RAM | 기준 메모리 | 이후 자동 튜닝 |
|------------|------------|----------------|
| ≥ 16 GB | 512 MB | → ~1s로 보정 |
| ≥ 8 GB | 256 MB | → ~1s로 보정 |
| ≥ 4 GB | 128 MB | → ~1s로 보정 |
...
```
Password → Argon2id(64-512MB memory) → 256비트 key → AES-256-GCM → Encrypted backup
```
...
🛡️ **무차별 대입 방지** — Argon2id는 시도당 64-512MB RAM 필요 (자동 조정)  
...
| **Argon2id KDF** | 64-512MB 메모리 (자동), 4회 반복, 1-4개 병렬 레인 |
...
| 알고리즘 | 매개변수 |
|------------|----------|
| **키 파생** | Argon2id (memory: 64-512MB 자동, iterations: 4, parallelism: 1-4) |
| **대칭 암호화** | AES-256-GCM |
| **솔트** | 32바이트 (암호학적 무작위) |
