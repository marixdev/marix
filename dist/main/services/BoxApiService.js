...
class BoxApiService {
     ...
     */
    static async getOrCreateBackupFolder(accessToken) {
        try {
            console.log('[Box API] Looking for Marix folder in root...');
            // Search for Marix folder in root (folder 0)
            const items = await this.apiRequest('GET', '/folders/0/items?fields=id,name,type', accessToken);
            const marixFolder = items.entries.find((item) => item.type === 'folder' && item.name === this.BACKUP_FOLDER_NAME);
            if (marixFolder) {
            ...
            const newFolder = await this.apiRequest('POST', '/folders', accessToken, {
     ...
     */
    static async getBackupFile(accessToken, folderId) {
        try {
            const items = await this.apiRequest('GET', `/folders/${folderId}/items?fields=id,name,type,size,modified_at`, accessToken);
            const backupFile = items.entries.find((item) => item.type === 'file' && item.name === this.BACKUP_FILE_NAME);
    ...
    static async uploadBackup(accessToken, encryptedData) {
        try {
            ...
            if (existingFile) {
                ...
                await this.uploadRequest('POST', `/files/${existingFile.id}/content`, accessToken, body, boundary);
                ...
                parts.push(Buffer.from(`--${boundary}--\r\n`));
                const body = Buffer.concat(parts);
                await this.uploadRequest('POST', '/files/content', accessToken, body, boundary);
     ...
     */
    static async uploadRequest(method, endpoint, accessToken, body, boundary) {
        return new Promise((resolve, reject) => {
...
exports.BoxApiService = BoxApiService;
BoxApiService.BOX_API_BASE = 'https://api.box.com/2.0';
BoxApiService.BOX_UPLOAD_BASE = 'https://upload.box.com/api/2.0';
BoxApiService.BACKUP_FOLDER_NAME = 'Marix';
BoxApiService.BACKUP_FILE_NAME = 'backup.marix';
//# sourceMappingURL=BoxApiService.js.map